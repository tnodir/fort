        ...
        int ipProtoField)
{
    ...
    if (app_flags.v == 0 ? conf_flags.allow_all_new
        ...
        if (conf_flags.log_stat) {
            ...
            status = fort_flow_associate(&g_device->stat, flow_id, process_id, group_index, is_tcp,
            ...
            if (!NT_SUCCESS(status)) {
                ...
                fort_buffer_proc_new_write(
                        &g_device->buffer, process_id, path_len, path, &irp, &info);
            }
        }
...
static void fort_callout_defer_packet_flush(UINT32 list_bits, BOOL dispatchLevel)
{
    fort_defer_packet_flush(
            &g_device->defer, fort_packet_inject_complete, list_bits, dispatchLevel);
}
...
static void fort_callout_defer_flush(void)
{
    ...
    if ((streamFlags
        ...
        if (fragment_flags != 0 && !(fragment_flags & FORT_FLOW_FRAGMENTED)) {
            ...
            if (fragment_flags & FORT_FLOW_FRAGMENT_DEFER) {
                const NTSTATUS status = fort_defer_stream_add(
                        &g_device->defer, inFixedValues, inMetaValues, streamData, filter, inbound);
    ...
    if (ignore_tcp_rst) {
      ...
      if (tcpFlags & TCP_FLAG_RST)
        ...
        if (defer_flow && NET_BUFFER_DATA_LENGTH(netBuf) == headerOffset) {
            const NTSTATUS status = fort_defer_packet_add(&g_device->defer, inFixedValues,
        ...
        if (fragment_packet) {
            fort_defer_stream_flush(
                    &g_device->defer, fort_packet_inject_complete, flow->flow_id, FALSE);
...
static NTSTATUS fort_callout_install(PDEVICE_OBJECT device)
{
    FWPS_CALLOUT0 c;
    NTSTATUS status;
...
static void fort_callout_remove(void)
{
...
static NTSTATUS fort_callout_force_reauth(
...
static void fort_callout_timer(void)
{
...
static NTSTATUS fort_device_create(PDEVICE_OBJECT device, PIRP irp)
{
...
static NTSTATUS fort_device_close(PDEVICE_OBJECT device, PIRP irp)
{
    UNUSED(device);
...
static NTSTATUS fort_power_callback_register(void)
{
    ...
    if (NT_SUCCESS(status)) {
        g_device->power_cb_reg =
                ExRegisterCallback(g_device->power_cb_obj, fort_power_callback, NULL);
    }
...
static NTSTATUS fort_systime_callback_register(void)
{
    ...
    if (NT_SUCCESS(status)) {
        g_device->systime_cb_reg =
                ExRegisterCallback(g_device->systime_cb_obj, fort_systime_callback, NULL);
    }
