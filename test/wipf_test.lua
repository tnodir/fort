...
do
  ...
  assert(not wipf.conf_ip_inrange(conf_ptr, 0, true))
  assert(not wipf.conf_ip_inrange(conf_ptr, sock.inet_pton("9.255.255.255", true)))
  assert(not wipf.conf_ip_inrange(conf_ptr, sock.inet_pton("11.0.0.0", true)))
  assert(wipf.conf_ip_inrange(conf_ptr, sock.inet_pton("10.0.0.0", true)))
  assert(wipf.conf_ip_inrange(conf_ptr, sock.inet_pton("169.254.100.100", true)))
  assert(wipf.conf_ip_inrange(conf_ptr, sock.inet_pton("192.168.255.255", true)))
  assert(not wipf.conf_ip_inrange(conf_ptr, sock.inet_pton("193.0.0.0", true)))

  assert(wipf.conf_app_blocked(conf_ptr, "System"))
  assert(not wipf.conf_app_blocked(conf_ptr, util_fs.path_to_dospath[[C:\programs\skype\phone\skype.exe]]))
  assert(not wipf.conf_app_blocked(conf_ptr, util_fs.path_to_dospath[[C:\utils\dev\git\bin\git.exe]]))
